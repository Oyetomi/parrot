...
// Groq's free tier rejects anything over 25 MB. 16 kHz mono 16-bit PCM runs
// 32 kB/s, so we split well before that and stitch the timestamps back.
...
export async function decode(file) {
  ...
  try {
    ...
    throw new UnsupportedFileError(
      "This browser could not decode that file's audio track."
    );
  } finally {
    ctx.close();
  }
 ...
 */
export function chunk(samples, rate = TARGET_RATE) {
  const per = MAX_CHUNK_SEC * rate;
  if (samples.length <= per) {
    return [{ blob: encodeWav(samples, rate), offset: 0 }];
  }
  const out = [];
  for (let start = 0; start < samples.length; start += per) {
    out.push({
      blob: encodeWav(samples.subarray(start, start + per), rate),
      offset: start / rate,
    });
  }
  return out;
...
export function envelope(samples, buckets = 340) {
  const per = samples.length / buckets;
  const out = new Array(buckets);
  let max = 0;
  for (let i = 0; i < buckets; i++) {
    const s = Math.floor(i * per), e = Math.min(samples.length, Math.floor((i + 1) * per));
    let sum = 0;
...
export function playableUrl(samples, rate = TARGET_RATE) {
